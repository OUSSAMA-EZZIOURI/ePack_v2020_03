...
public class S002_FORS_PN implements ControlState {
    ...
    public void doAction(WarehouseControlContext context) {
        ...
        //Check the PN format in the pattern list
        if (!WarehouseHelper.checkDispatchPNFormat(scan_txtbox.getText().trim())) {
            ...
            clearScanBox(scan_txtbox);
                ...
                !bc.getHarnessPart().equals(scan_txtbox.getText().trim())
                &&                 
                !bc.getHarnessPart().equals(scan_txtbox.getText().trim().substring(1))
                //&&
                
                ) { // Check the bc.PN and scanned part number
            SoundHelper.PlayErrorSound(null);
            ...
            S003_FORS_QTY state = new S003_FORS_QTY(this.bc);
