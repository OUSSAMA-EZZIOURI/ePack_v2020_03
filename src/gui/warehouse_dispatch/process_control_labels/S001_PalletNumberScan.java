    ...
    public void doAction(WarehouseControlContext context) {
        ...
        if (scan_txtbox.getText().startsWith(GlobalVars.CLOSING_PALLET_PREFIX)) {
            ...
            if (result.isEmpty()) {
                ...
                if (!this.bc.getContainerState().equals(GlobalVars.PALLET_RESERVED)) {
                    SoundHelper.PlayErrorSound(null);
                    String msg = "Etat de la palette " + bc.getContainerState() + ". La palette " + this.bc.getPalletNumber() + " doit être à l'état " + GlobalVars.PALLET_RESERVED + ".";
                    WarehouseHelper.Label_Control_Gui.setMessageLabel(msg, -1);
                    ...
                    //Checker si la palette est déjà associée à ce plan
                    Helper.startSession();
                    query = Helper.sess.createQuery(HQLHelper.GET_LOAD_PLAN_LINE_BY_PAL_NUM_AND_PLAN_ID);
                    query.setParameter("palletNumber", this.bc.getPalletNumber());
                    query.setParameter("loadPlanId", WarehouseHelper.temp_load_plan.getId());
                    Helper.sess.getTransaction().commit();
                    System.out.println("Query yahya ibn ibrahim al judali "+query.toString());
                    result = query.list();
                    //Reload LoadPlan line details
                    if (result.isEmpty()) {
                        SoundHelper.PlayErrorSound(null);
                        String msg = "Cette palette ne figure pas de ce plan de chargement "+WarehouseHelper.temp_load_plan.getId().toString()+" !";
                        WarehouseHelper.Label_Control_Gui.setMessageLabel(msg, -1);
                        JOptionPane.showOptionDialog(null, msg, "Erreur de scanne.", JOptionPane.DEFAULT_OPTION, JOptionPane.ERROR_MESSAGE, null, new Object[]{}, null);
                        clearScanBox(scan_txtbox);
                    }else{
                                ...
                                if (lp != null) {
                                    ...
                                    WarehouseHelper.Label_Control_Gui.setTxt_productQty(
                                            new DecimalFormat("00.00").format(this.bc.getQtyReadDouble())
                                            );
