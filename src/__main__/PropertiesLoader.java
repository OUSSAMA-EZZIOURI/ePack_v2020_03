...
public class PropertiesLoader {
     ...
     */
    @SuppressWarnings("CallToThreadDumpStack")
    public static void createDailyOutPrintDir(String print_dir_path, String open_pallet, String closed_pallet, String picking_dir, String dispatch_dir) {
        String msg = "";
        // if the printing output printing
        File print_dir = new File(print_dir_path);
        if (!print_dir.exists()) {
        ...
        if (!today_print_dir.exists()) {
            today_print_dir.mkdir();
            msg += "Daily output print directory [" + today_print_dir.getPath() + "] created !\n";
        } else {
            msg += "Daily output print directory [" + today_print_dir.getPath() + "] already exist!\n";
        }
        
        System.out.println("today_print_dir.getPath() "+today_print_dir.getPath());
        System.out.println("palet_dir "+open_pallet);
        
        //Create PRINT_PALLET_DIR       
        String palet_dir_path = today_print_dir.getPath() + open_pallet + File.separator;
        System.out.println("palet_dir_path "+palet_dir_path);
        File file1 = new File(palet_dir_path);
        if (!file1.exists()) {
            file1.mkdir();
            msg += "open_sheet sub directory [" + file1.getPath() + "] created !\n";
        } else {
            msg += "open_sheet sub directory [" + file1.getPath() + "] already exist!\n";
        }

        //Create galia sub directory
        String galia_dir_path = today_print_dir.getPath() + closed_pallet + File.separator;
    ...
    public static String loadConfigProperties() {
        ...
        try {
            input = new FileInputStream("."+File.separatorChar+"config.properties");
