...
public class Mode2_S031_PalletChoice implements Mode2_State {
    ...
    public void doAction(Mode2_Context context) {
        ...
        if (!barcode.isEmpty()) {
            ...
            } //##################### IF THE PALLET IS BLOQUED ###################
            else if (bc.getContainerStateCode().equals(GlobalVars.PALLET_BLOCKED_CODE)) {
                String APP_ERR0050 = ErrorMsg.APP_ERR0050A[1]+" "+bc.getPalletNumber()+" "+ErrorMsg.APP_ERR0050B[1];
                UILog.severe(APP_ERR0050);
                UILog.severeDialog(null, APP_ERR0050, ErrorMsg.APP_ERR0050A[0]);
                PackagingVars.Packaging_Gui_Mode2.getFeedbackTextarea().setText(APP_ERR0050);
            ...
            } //########### CHECK THE 5 CONDITIONS OF AN OPEN PALLET #############
            ...
            //# 5- Container Harness Type = Mode2_Context Harness Type            
                ...
                if (bc.getQtyExpected() == newQty || bc.getQtyExpected() == 1) {
                    ...
                    for (int copies = 0; copies < bc.getClosingSheetCopies(); copies++) {
                        ...
                        try {
                            //UILog.infoDialog(""+this.getClass().getName()+" 120 "+bc.getClosingSheetFormat());
                            PrinterHelper.saveAndPrintClosingSheet(PackagingVars.mode2_context, bc, false, bc.getClosingSheetFormat());
                        } catch (IOException ex) {
                            UILog.severe(ex.toString());
                        } catch (DocumentException ex) {
                            UILog.severe(ex.toString());
                        } catch (Exception ex) {
                            Logger.getLogger(Mode2_S031_PalletChoice.class.getName()).log(Level.SEVERE, null, ex);
                        }
