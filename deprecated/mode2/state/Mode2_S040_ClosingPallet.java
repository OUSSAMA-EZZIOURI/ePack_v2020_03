...
public class Mode2_S040_ClosingPallet implements Mode2_State {
    ...
    public void doAction(Mode2_Context context) {
        ...
        if (bc.getContainerStateCode().equals(GlobalVars.PALLET_BLOCKED_CODE)) {
            String APP_ERR0051 = ErrorMsg.APP_ERR0051A[1] + " " + bc.getPalletNumber() + " " + ErrorMsg.APP_ERR0051B[1] + " " +GlobalVars.PALLET_CLOSED;
            UILog.severe(APP_ERR0051);
            UILog.severeDialog(null, APP_ERR0051, ErrorMsg.APP_ERR0051A[0]);
            PackagingVars.Packaging_Gui_Mode2.getFeedbackTextarea().setText(APP_ERR0051);
        ...
        } else if (!barcode.isEmpty() && barcode.equals(GlobalVars.CLOSING_PALLET_PREFIX + context.getBaseContainerTmp().getPalletNumber())) {
